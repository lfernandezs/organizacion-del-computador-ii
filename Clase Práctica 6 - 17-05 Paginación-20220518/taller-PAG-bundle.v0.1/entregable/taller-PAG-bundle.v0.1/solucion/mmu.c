...
paddr_t mmu_init_kernel_dir(void) {
  for (int i = 0; i < 1024; i++) {
    pt_entry_t kpt_entry = {
      .page = (next_free_kernel_page+i*0x1000), // +i*4kib 
      .attrs = 0x3
    };
  ...
  pd_entry_t kpd_entry = {
    .pt = kpt,
    .attrs = 0x3
  };

  kpd[0] = kpd_entry; // TODO: ¿Por qué inicializo solo una pd entry?
...
void mmu_map_page(uint32_t cr3, vaddr_t virt, paddr_t phy, uint32_t attrs) {
  ...
  pd_entry_t pd_entry = pd[VIRT_PAGE_DIR(virt)];

  pd_entry.attrs = attrs;

  pt_entry_t* pt = pd_entry.pt;
  pt_entry_t pt_entry = pt[VIRT_PAGE_TABLE(virt)];

  pt_entry.attrs = attrs;
  pt_entry.page = phy >> 12;
